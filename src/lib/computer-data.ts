...
/**
 * detectConditionFromText — פונקציה סמכותית ויחידה לזיהוי מצב מוצר מטקסט.
 * מחזירה ערך מדויק מתוך CONDITION_OPTIONS.
 * ⚠️ מקור האמת היחיד: אין לשכפל לוגיקת מצב בקובץ אחר!
 *
 * סדר הבדיקות: מהכי ספציפי לכי כללי.
 * שימוש ב-RegEx (לא includes!) להימנע מבאגים כמו "כחדש" ← "חדש".
 */
export function detectConditionFromText(text: string): string {
    // 1. חדש באריזה / Sealed — הכי ספציפי
    if (/חדש\s*באריזה|חדש\s*בניילון|sealed|אריזה\s*מקורית|אריזה\s*סגורה|brand[\s-]*new/i.test(text))
        return "חדש";

    // 2. כמו חדש — חייב לפני "חדש" סתם!
    // כולל: כחדש / כמו חדש / שמור כחדש / שמורה כחדשה / like new / מצב מצוין / 10/10
    if (/כמו\s*חדש|כחדש|שמור[ה]?\s*כחדש[ה]?|like[\s-]*new|10\/10|9\.5\/10|מצב\s*מצוין(?!\s*-)/i.test(text))
        return "כמו חדש";

    // 3. חדש גרידא — מוודאים שאינו חלק מ"כחדש", "מחדש", "לחדש", "החדש"
    // שימוש ב-(?:^|[\s,.-]) כתחליף ל-\b (שלא עובד טוב בעברית)
    if (/(?:^|[\s,.-])חדש(?:[\s,.-]|$)|(?:^|\s)new(?:\s|$)/i.test(text))
        return "חדש";

    // 4. משומש - מצב מצויין: 9/10, 8.5/10, מצב מצוין עם מקף
    if (/9\/10|8\.5\/10|מצב\s*מצוין|מצב\s*מעולה|מצב\s*יוצא\s*מהכלל/i.test(text))
        return "משומש - מצב מצויין";

    // 5. משומש - מצב טוב: 8/10, 7.5/10, שמור, מצב טוב, מצב יפה
    if (/8\/10|7\.5\/10|מצב\s*טוב|מצב\s*יפה|שמור[ה]?(?!\s*כחדש)|בסדר\s*גמור/i.test(text))
        return "משומש - מצב טוב";

    // 6. משומש - מצב בינוני: 7/10, 6/10, סימני שימוש, שריטות, פגמים
    if (/7\/10|6\/10|משומש|יד\s*(?:שנייה|שניה|2)|סימני\s*שימוש|שריטות?|פגמים?|בלאי/i.test(text))
        return "משומש - מצב בינוני";

    // 7. לחלקים / לא עובד
    if (/לחלקים|לא\s*עובד|תקול|שבור|מקולקל/i.test(text))
        return "לחלקים / לא עובד";

    // ברירת מחדל: מצב טוב (רוב המוצרים בשוק)
    return "משומש - מצב טוב";
}
