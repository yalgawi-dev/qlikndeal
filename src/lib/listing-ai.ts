...
export function analyzeListingText(text: string, options?: AiOptions): ListingAnalysis {
    ...
    const requiredByCategory: Record<string, string[]> = {
        "אלקטרוניקה": ["נפח אחסון", "גודל מסך", "מצב סוללה", "שנת ייצור"],
        "רכב": ["שנת ייצור", "קילומטראז'", "סוג דלק", "יד", "טסט עד"],
        "ריהוט": ["מידות", "צבע", "מצב", "חומר"],
        "נדלן": ["מטראז'", "חדרים", "קומה", "כתובת"],
...
const CATEGORIES: Record<string, { keywords: string[]; subCategories: Record<string, string[]> }> = {
    "אלקטרוניקה": {
        keywords: ["אייפון", "סמסונג", "שיאומי", "אנדרואיד", "גלקסי", "pixel", "מחשב", "לפטופ", "נייד", "מקבוק", "טאבלט", "אייפד", "טלוויזיה", "מסך", "מקרן", "OLED", "QLED", "4K", "אוזניות", "רמקול", "בלוטוס", "bluetooth", "מצלמה", "DSLR", "gopro", "פלייסטיישן", "אקסבוקס", "נינטנדו", "PS4", "PS5", "גיימינג", "שעון חכם", "דרון", "wifi", "קריוקי", "מיקרופון"],
        subCategories: {
            "טלפונים": ["אייפון", "סמסונג", "שיאומי", "אנדרואיד", "גלקסי"],
            "מחשבים": ["מחשב", "לפטופ", "מקבוק", "notebook", "PC"],
            "טלוויזיות ומסכים": ["טלוויזיה", "מסך", "OLED", "QLED", "4K", "מקרן"],
            "אודיו": ["רמקול", "אוזניות", "סאונד", "בלוטוס", "קריוקי", "מיקרופון"],
            "משחקים": ["פלייסטיישן", "אקסבוקס", "נינטנדו", "גיימינג", "PS4", "PS5"],
            "מצלמות": ["מצלמה", "DSLR", "gopro", "דרון"],
        },
    },
    "ריהוט": {
        keywords: ["מיטה", "ספה", "ארון", "שולחן", "כסא", "מזנון", "שידה", "כורסה", "מדף", "ספרייה", "מזרן", "קומודה", "פינת אוכל"],
        subCategories: {
            "שינה": ["מיטה", "מזרן", "שידה", "ארון בגדים"],
            "סלון": ["ספה", "כורסה", "מזנון", "שולחן קפה"],
            "פינת אוכל": ["שולחן אוכל", "כסאות", "פינת אוכל"],
            "משרד": ["שולחן עבודה", "כסא משרדי", "ספרייה"],
        },
    },
    "רכב": {
        keywords: ["טויוטה", "מזדה", "הונדה", "יונדאי", "קיה", "BMW", "מרצדס", "פולקסווגן", "אאודי", "פורד", "ניסן", "סובארו", "לקסוס", "קילומטר", "קמ", "טסט", "ידני", "אוטומט", "דיזל", "בנזין", "היברידי", "חשמלי", "אופנוע", "קטנוע"],
        subCategories: {
            "רכב פרטי": ["סדאן", "האצ'בק", "טויוטה", "מזדה", "הונדה"],
            "ג'יפ / SUV": ["ג'יפ", "SUV", "4X4"],
            "אופנוע": ["אופנוע", "קטנוע", "סקוטר"],
        },
    },
    "ביגוד ואופנה": {
        keywords: ["חולצה", "מכנסיים", "שמלה", "נעליים", "תיק", "תכשיט", "שעון", "ג'קט", "מעיל", "סוודר", "XS", "XL", "XXL", "מידה", "nike", "adidas", "zara"],
        subCategories: {
            "ביגוד": ["חולצה", "מכנסיים", "שמלה", "ג'קט", "מעיל"],
            "נעלים": ["נעליים", "סניקרס", "מגפיים"],
            "תיקים": ["תיק", "תיק יד", "תיק גב", "ארנק"],
            "תכשיטים": ["תכשיט", "טבעת", "צמיד", "עגיל"],
        },
    },
    "ספורט ופנאי": {
        keywords: ["אופניים", "קורקינט", "כושר", "משקולות", "הליכון", "כדור", "טניס", "כדורגל", "קמפינג", "אוהל"],
        subCategories: {
            "אופניים וקורקינטים": ["אופניים", "קורקינט"],
            "כושר": ["משקולות", "הליכון", "אליפטי"],
            "קמפינג": ["אוהל", "שק שינה", "תרמיל"],
        },
    },
    "זיהוי כפול": { // Placeholder to avoid breaking loop if needed, though this is cleaner:
        keywords: [], // Empty
        subCategories: {},
    },
    // Fix: Remove generic "For Sale/Rent" and "Room" from Real Estate to avoid false positives in Furniture (Bedroom) or General (For Sale ...)
    "נדלן": {
        keywords: ["דירה", "בית פרטי", "וילה", "פנטהאוז", "דופלקס", "יחידת דיור", "מ\"ר", "קומה", "מרפסת", "חניה פרטית", "ממ\"ד", "טאבו", "נדלן", "מגרש", "סטודיו להשכרה", "שותפים"],
        subCategories: {
            "למכירה": ["למכירה"],
            "להשכרה": ["להשכרה", "שכירות", "שותפים"],
        },
    },
    "בית וגינה": {
        keywords: ["מקרר", "מדיח", "מכונת כביסה", "תנור", "מיקרוגל", "מזגן", "מאוורר", "שואב אבק", "מכונת קפה", "בלנדר", "פריזר", "מקדחה", "גינה", "מנורה", "תאורה", "גוף תאורה", "אהיל", "כלי עבודה", "מברגה"],
        subCategories: {
            "מוצרי חשמל גדולים": ["מקרר", "מכונת כביסה", "מדיח", "מזגן", "תנור"],
            "מוצרי חשמל קטנים": ["מכונת קפה", "בלנדר", "מיקסר", "שואב אבק", "מיקרוגל"],
        },
    },
    "כלי נגינה": {
        keywords: ["גיטרה", "פסנתר", "תופים", "בס", "מקלדת מוזיקלית", "מגבר", "כינור", "חליל", "סקסופון"],
        subCategories: {
            "מיתרים": ["גיטרה", "בס", "כינור"],
            "הקשה": ["תופים", "קאחון"],
            "קלידים": ["פסנתר", "מקלדת מוזיקלית"],
        },
    },
    "ספרים ומדיה": {
        keywords: ["ספר", "ספרים", "תקליט", "DVD", "CD", "ויניל", "מגזין"],
        subCategories: {
            "ספרים": ["ספר", "רומן"],
            "מדיה": ["DVD", "CD", "תקליט", "ויניל"],
        },
    },
};
