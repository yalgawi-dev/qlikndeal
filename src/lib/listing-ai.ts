...
function extractHighlights(text: string, options?: AiOptions): string[] {
    ...
    const patterns = [
        ...
        /מימון\s*עד\s*\d+%/i,
        /ליסינג/i,
        // Ownership duration — trust signal ("נקנה לפני שנה וחודש", "נקנה לפני ארבע חודשים")
        /נקנה?\s*לפני\s*(?:[\u05D0-\u05EA]+\s*|\d+\s*)?(?:חודשים|שניים|שנים|שבועות|שנה|חודש|שבוע)(?:\s*ו(?:חודשים|שבועיים|חודש|שבוע)?)?/i,
        /בשימוש\s*(?:פחות\s*מ)?(?:שנה|חודשיים|חודשים|חודש|\d+\s*(?:שנ[הות]|חודש(?:ים)?))/i,
        /(?:קנתי|קניתי|נרכש)\s*לפני\s*(?:[\u05D0-\u05EA]+\s*|\d+\s*)?(?:חודשים|שניים|שנים|שבועות|שנה|חודש|שבוע)/i,
        // Condition / Bonus extras
        /במצב\s*חדש(?:\s*לגמרי)?/i,
        /(?:נרתיק|עט)\s*(?:ו(?:נרתיק|עט)\s*)?כלולים/i,
        /כמתנה/i,
